# -*- coding: utf-8 -*-
"""
Created on Tue Jun  8 14:21:09 2021
...
def main():
    ...
    for r0 in r0:
        ...
        i2 = len(results_array[4])
        check2 = np.arange(i2)
        remove2 = np.empty(0, dtype=int)
        for x in check2:
            if results_array[4,x] > (results_array[2,x]+0.00737024)/0.8391:
        ...
        coord = [[0,0],[0,.007],[-.003,.005],[-.003,.00035],[-.0034,.00035],[-.0034,.01],[-0.0144,.01],[-0.0144,.0047128],[-.0064,-.002],[-.0064,-.01],[-.0034,-.01],[-.0034,-.00035],[-.003,-.00035],[-.003,-.005],[0,-.007]]
