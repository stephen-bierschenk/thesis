# -*- coding: utf-8 -*-
"""
Created on Thu Jun  3 11:46:02 2021
...
def main():
    ...
    for Dnp in Dnp:
        ...
        coord = [[0,0],[0,.007],[-.003,.005],[-.003,.00035],[-.0034,.00035],[-.0034,.01],[-0.0064,.01],[-0.0064,-.01],[-.0034,-.01],[-.0034,-.00035],[-.003,-.00035],[-.003,-.005],[0,-.007]]
        ...
        plt.plot(-(dz*1000-3), dz_dt)
        plt.axis([3.5,-3.9,0,500])
        plt.axvline(x=-3.4, color='k')
        plt.axvline(x=-2.55, color='k')
        plt.text(-1,80,'Stagnation \nRegion')
        plt.text(-1.5,425,'200 nm')
        plt.text(-1.5,355,'600 nm')
        plt.text(-1.5,280,'1 µm')
        plt.text(-1.5,215,'1.4 µm')
        plt.arrow(-2.2,97,-.5,0,width=1,head_width=7, head_length=.1, shape='full',fc='k')
        plt.ylabel('Normal Velocity (m/s)',fontsize=16)
        plt.xlabel('Axial Location (mm)',fontsize=16)
        plt.savefig('Normal Velocity Drop 0deg.png', dpi=1200)
