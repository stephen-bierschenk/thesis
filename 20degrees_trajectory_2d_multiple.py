...
def main():
    ...
    #fg, ax = plt.subplots(1,1,dpi=1200)
    ...
    for r0 in r0:
        ...
        coord = [[0*1000+3.4,0],[0*1000+3.4,.007*1000],[-.003*1000+3.4,.005*1000],[-.003*1000+3.4,.00035*1000],[-.0034*1000+3.4,.00035*1000],[-.0034*1000+3.4,.01*1000],[-0.0100397*1000+3.4,.01*1000],[-0.0100397*1000+3.4,.008*1000],[-.0064*1000+3.4,-.002*1000],[-.0064*1000+3.4,-.01*1000],[-.0034*1000+3.4,-.01*1000],[-.0034*1000+3.4,-.00035*1000],[-.003*1000+3.4,-.00035*1000],[-.003*1000+3.4,-.005*1000],[0+3.4,-.007*1000]]
        coord.append(coord[0]) #repeat the first point to create a 'closed loop'
    
        xs, ys = zip(*coord) #create lists of x and y values
        # plt.plot(dr*1000, -dz*1000+3.4, markersize=1) #r,-z,'o',
        # plt.plot(ys,xs,color='b')
        # plt.ylabel('Axial Position (mm)', fontsize=16)
        # plt.xlabel('Radial Position (mm)',fontsize=16)
                
        pac=mpatches.Arc([8,-6.6397],7,7,angle=0, theta1=160, theta2=180,color='b')
        ...
        # ax.set_aspect(4/3)
        # fg.canvas.draw()
        # plt.savefig('Trajectories_1000nm_20deg.png')
        plt.figure(1)
        plt.plot(dr*1000, -dz*1000+3.4, markersize=1) #r,-z,'o',
        plt.plot(ys,xs,color='b')
        plt.ylabel('Axial Position (mm)', fontsize=16)
        plt.xlabel('Radial Position (mm)',fontsize=16)
        #matplotlib.patches.Arc((-0.0100397*1000+3.4,.008*1000),3,3,angle=20)
        #plt.savefig('Trajectories_600nm_20deg.png', dpi=1200)
