...
def main():
   ...
   # r0 = float(r0_entry.get())
    ...
    for r0 in r0:
        ...
        for x in check:
            if results_array[4,x] > (results_array[2,x]+0.01391030054899042)/1.860984460779753:
        ...
        coord = [[0,0],[0,.007],[-.003,.005],[-.003,.00035],[-.0034,.00035],[-.0034,.01],[-.0117735,.01],[-.0117735,.008],[-.0064,-.002],[-.0064,-.01],[-.0034,-.01],[-.0034,-.00035],[-.003,-.00035],[-.003,-.005],[0,-.007]]
...
tot_time_entry.insert(0,'0.0004') # default time for ode solver
